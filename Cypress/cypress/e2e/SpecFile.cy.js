...
describe("Automation Suite for Yaksha Application", () => {
  ...
  it("TS-8 Verify Patient Search Functionality with Multiple Patients", () => {
  ...
  it("TS-10 Modular Script for Patient Search", () => {
  ...
  it("TS-12 Verify 'New Visit' tab opens with Alt + N keyboard shortcut in Appointment Page", async () => {
    appointment.openNewVisitPageThroughKeyboardButton();
    verifyVisitPageOpens();
  });

  it("TS-13 Verify the tooltip text on hover of Star icon in Laboratory", () => {
    laboratoryPage.verifyStarTooltip();
    verifyUserIsOnCorrectURL("Lab/Dashboard");
  });

  it('TS-14 Add and Verify New Imaging Type in Radiology', () => {
    settingsPage.addAndVerifyNewImagingType();
    verifyImagingTypeAdded();
  });

  it("TS-15 Web Element Handling for Dropdowns in Purchase Request", () => {
    procurement.verifyRequestedDateColumnDateWithinRange();
    verifyPurchaseReqDataIsPresent();
  });

  it("TS-16 Login with invalid credentials", () => {
